...
class set_values(nn.Module):
    ...
    def forward(self, seq, xinp):
        xout = Variable(
            torch.zeros(xinp.size()[0], xinp.size()[1], self.hidden_size, self.height, self.width)
        ).to(device)

        h_state, c_state = (
            Variable(torch.zeros(xinp[0].shape[0], self.hidden_size, self.height, self.width)).to(device),
            Variable(torch.zeros(xinp[0].shape[0], self.hidden_size, self.height, self.width)).to(device)
        )
