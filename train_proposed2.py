...
def run_training(cfg: experiment_manager.CfgNode):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(dataloader):
            ...
            timestamp_combinations = list(combinations(range(cfg.DATALOADER.TIMESERIES_LENGTH), 2))
            ...
            for i_comb, (t1, t2) in enumerate(timestamp_combinations):
                y_ch = torch.ne(y[:, t1], y[:, t2])
                loss_ch += criterion(logits_ch[:, i_comb], y_ch)
        ...
        # _ = evaluation.model_evaluation_proposed2(net, cfg, device, 'train', epoch_float, global_step)
        f1_val = evaluation.model_evaluation_proposed2(net, cfg, device, 'val', epoch_float, global_step)
    ...
    _ = evaluation.model_evaluation_proposed2(net, cfg, device, 'test', epoch_float, global_step)
