...
class TrainTransformerDataset(AbstractSpaceNet7Dataset):
...
class EvalTransformerDataset(AbstractSpaceNet7Dataset):
    ...
    def __init__(self, cfg: experiment_manager.CfgNode, run_type: str, tiling: int = None, aoi_id: str = None):
        ...
        if aoi_id is None:
            if run_type == 'train':
                self.aoi_ids = list(cfg.DATASET.TRAIN_IDS)[:2]
            elif run_type == 'val':
                self.aoi_ids = list(cfg.DATASET.VAL_IDS)
            elif run_type == 'test':
                self.aoi_ids = list(cfg.DATASET.TEST_IDS)
        ...
        self.samples = []
        for aoi_id in self.aoi_ids:
            for i in range(self.tiling // 2, 1024 - self.tiling // 2):
                for j in range(self.tiling // 2, 1024 - self.tiling // 2):
                    self.samples.append((aoi_id, (i, j)))
    ...
    def __getitem__(self, index):
        ...
        images = [self.load_planet_mosaic(ts['aoi_id'], ts['dataset'], ts['year'], ts['month']) for ts in timestamps]
        offset = self.tiling // 2
        images = np.stack(images)[:, i - offset:i + offset + 1, j - offset:j + offset + 1]
        labels = [self.load_building_label(ts['aoi_id'], ts['year'], ts['month']) for ts in timestamps]
        labels = np.stack(labels)[:, i - offset:i + offset + 1, j - offset:j + offset + 1]
