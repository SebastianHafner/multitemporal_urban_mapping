...
class DecoderTransformer(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
                 in_channels=[32, 64, 128, 256], embedding_dim=64, output_nc=2, feature_strides=[2, 4, 8, 16]):
        ...
        self.probability = ConvLayer(self.embedding_dim, self.output_nc, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self._transform_inputs(x)  # len=4, 1/2, 1/4, 1/8, 1/16
        ...
        outputs.append(p_c4)
        _c4_up = resize(_c4, size=c1.size()[2:], mode='bilinear', align_corners=False)

        # Stage 3: x1/16 scale
        _c3 = self.linear_c3(c3).permute(0, 2, 1).reshape(n, -1, c3.shape[2], c3.shape[3])
        _c3 = _c3 + F.interpolate(_c4, scale_factor=2, mode="bilinear")
        p_c3 = self.make_pred_c3(_c3)
        outputs.append(p_c3)
        _c3_up = resize(_c3, size=c1.size()[2:], mode='bilinear', align_corners=False)

        # Stage 2: x1/8 scale
        _c2 = self.linear_c2(c2).permute(0, 2, 1).reshape(n, -1, c2.shape[2], c2.shape[3])
        _c2 = _c2 + F.interpolate(_c3, scale_factor=2, mode="bilinear")
        p_c2 = self.make_pred_c2(_c2)
        outputs.append(p_c2)
        _c2_up = resize(_c2, size=c1.size()[2:], mode='bilinear', align_corners=False)

        # Stage 1: x1/4 scale
        _c1 = self.linear_c1(c1).permute(0, 2, 1).reshape(n, -1, c1.shape[2], c1.shape[3])
        _c1 = _c1 + F.interpolate(_c2, scale_factor=2, mode="bilinear")
...
class SegFormer(nn.Module):
    ...
    def __init__(self, cfg: experiment_manager.CfgNode):
        ...
        self.encoder = EncoderTransformer(
            img_size=256, patch_size=7, in_chans=input_nc, num_classes=output_nc,
            embed_dims=self.embed_dims, num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            qk_scale=None, drop_rate=self.drop_rate, attn_drop_rate=self.attn_drop, drop_path_rate=self.drop_path_rate,
